...
import InputSearchList from "../FormElements/inputSearchList";
...
const Header2 = (props: {
  ...
  const storeListClickFnc = (item : any)=>{
    setCurrent_store_code(item.store_code)
    setCurrent_store_name(item.store_name)
    router.push("/store/"+item.store_code+"/dashboard")
  }
      ...
      <div className="flex flex-grow items-center justify-between px-4 py-4 shadow-2 md:px-6 2xl:px-11">
        <div className="flex items-center gap-2 sm:gap-4 lg:hidden">
          ...
          <button
            ...
            className="z-99999 block rounded-sm border border-stroke bg-   p-1.5 shadow-sm dark:border-strokedark dark:bg-boxdark lg:hidden"
          >
          ...
          <form action="https://formbold.com/s/unique_form_id" method="POST">
            <div className="relative">
              {/* <button className="absolute left-0 top-1/2 -translate-y-1/2">
               ...
               <div className="w-150">
                {authState?.role_type == "admin" ? <InputSearchList id={"header_inputserach"} subject2={"이동할 스토어 검색 - 클릭시 이동, all 검색시 전체 리스트  나옴"} searchFnc={inputSearchFnc} itemClickFnc={storeListClickFnc}></InputSearchList> : ''}
               </div>
